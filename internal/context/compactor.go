...
// Compactor 负责监控和压缩上下文内存，防止大模型发生OOM
type Compactor struct {
	MaxChars       int // 触发压缩的最大字符数阈值（水位线，可参考使用的大模型的token窗口大小）
	RetainLastMsgs int // WorkingMemory 保护区：最近的 N 条消息
}
...
func (c *Compactor) Compact(msgs []schema.Message) []schema.Message {
	...
	for i, msg := range msgs {
		// 1. 系统提示词（SystemPrompt）绝对不能动，直接保留
		...
		if msg.Role == schema.RoleUser && msg.ToolCallID != "" {
			...
			if !isInWorkingMemory {
				...
				// 我们保留前 500 字符和后 500 字符（掐头去尾法，大模型通常只需要看开头报错和结尾报错）
				const maxKeep = 1000
		...
		// 注意：我们绝不会去动 msg.ToolCalls，因为这是模型行动的证据，是维系逻辑练得关键
