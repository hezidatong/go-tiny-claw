...
type Session struct {
	...
	history []schema.Message // 存放此 Session 中所有的用户输入、大模型回复和工具调用结果
	mu      sync.RWMutex     // 读写锁，防止并发读写历史时发生 DataRace
}
...
func (s *Session) GetWorkingMemory(limit int) []schema.Message {
	...
	// 但发出这个请求的 ToolCall 被我们截断抛弃了，大模型 API 会直接报 400 BadRequest。
