...
// RecoveryManager 负责在工具执行失败时，根据报错特征分析并注入回复建议
...
func (rm *RecoveryManager) AnalyzeAndInject(toolName string, rawError string) string {
	...
	switch toolName {
	case "edit_file":
		// 匹配 fuzzyReplace 的固定报错抛出
		if strings.Contains(rawError, "在文件中未找到 old_text") || strings.Contains(rawError, "找不到该代码片段") {
			hint = "你提供的 old_text 与文件当前内容不一致，或者缺少必要的缩进。请使用 `read_file` 工具重新读取该文件，获取最新、准确的内容后，再重新发起编辑。"
		} else if strings.Contains(rawError, "匹配到了多处") || strings.Contains(rawError, "提供更多上下文") {
			hint = "你的 old_text 不够具体，命中了多个相同的代码块。请在 old_text 中增加上下相邻的几行代码，以确保替换的唯一性。"
		}
	...
	case "bash":
		if strings.Contains(lowerError, "command not found") {
			hint = "系统中未安装命令。请先思考：是否有替代命令？或者你需要先编写脚本进行安装？"
